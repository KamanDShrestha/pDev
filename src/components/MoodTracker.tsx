  ...
  return (
    <>
      <div className="relative space-y-5 border rounded-xl max-w-[500px] p-5 shadow-lg  bg-stone-50 dark:bg-slate-900">
        <span className="absolute top-4 right-5 hover:cursor-pointer" onClick={handleModalClose}>
          X
        </span>
        <div>
          ...
          <div className="flex flex-wrap justify-center gap-5">
            {moods.map((mood, index) => (
              <div className={`flex flex-col items-center ${selectedMood === index && "border-2 border-slate-600 dark:border-white rounded-xl"} p-3 cursor-pointer`} key={index} onClick={() => handleMoodSelection(index)}>
        ...
        <div>
          <label className="text-lg font-medium">State your reasoning for your mood today. Reflect on that.</label>
          <Textarea
          ...
          />
          {errors.reasoning && <ErrorMessage>{errors.reasoning.message as string}</ErrorMessage>}
        </div>
