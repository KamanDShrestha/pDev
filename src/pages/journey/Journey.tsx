...
const Journey = () => {
  ...
  const { data: embarkedJourneys } = useGetAllEmbarkedJourneys(user?.id as string)
  ...
  return (
    <div>
      ...
      <div className="relative mt-8">
        ...
        <div className="flex flex-wrap justify-center gap-10">
          ...
          {journeys &&
            ...
            journeys
              ...
              .map((journey, index) => (
                <div key={index}>
                  <JourneyCard journeyId={journey._id} journeyName={journey.name} journeyDescription={journey.description} journeyIcon={journey.imageLinks} journeyLength={journey.length} importance={journey.importance} usages={journey.usages} />
                </div>
              ))}
        ...
        <p className="my-10 text-right">
          <span className="text-xs">
            *For medical emergencies or conditions, please seek appropriate medical assistance and expertise. <br />
            Our platform is designed to complement personal development efforts, not to provide medical advice or treatment.
          </span>
        </p>
      </div>
      ...
      <div className="mt-8">
        ...
        <div className="flex flex-wrap justify-center gap-10">{embarkedJourneys && embarkedJourneys.filter((embarkedJourney) => embarkedJourney.isJourneyCompleted === true).length !== 0 ? embarkedJourneys.map((embarkedJourney) => embarkedJourney.isJourneyCompleted && <RetrospectionCard key={embarkedJourney._id} keyLearning={embarkedJourney.keyLearning} reflection={embarkedJourney.reflection} embarkedJourneyId={embarkedJourney._id} journeyId={embarkedJourney.journeyId} journeyName={embarkedJourney.journeyName} />) : <p>You have not completed any journeys. </p>}</div>
      </div>
    </div>
  )
}
