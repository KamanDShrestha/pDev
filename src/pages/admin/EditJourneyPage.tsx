...
const EditJourneyPage = () => {
  ...
  const { data: journey, isLoading } = useGetSpecificJourney({ id })
  ...
  function onSubmit(data: FieldValues) {
    ...
    const actionSteps = Object.keys(data.actionSteps).reduce((acc, key) => {
      const { actionStep, description, evidences, additionalSteps, example } = data.actionSteps[key]
      ...
      return {
        ...
        [key]: {
          ...
          description,
          example,
          evidences: evidences.filter((evidence: string) => evidence !== null && evidence !== ""),
          additionalSteps: additionalSteps.filter((additionalStep: string) => additionalStep !== null && additionalStep !== ""),
        },
      }
    }, {})
      ...
      {journey && (
        <div className="space-y-5">
          <div className="flex flex-wrap justify-around gap-5">
            ...
            <div>
              <Heading className="mb-2 text-2xl">Length of the journey: {journey.length}</Heading>
                ...
                <>
                  {journey.importance.map((item, index) => (
                    <>
                      ...
                      />
                      {errors?.importance && errors.importance[index] && <ErrorMessage>{errors.importance[index]?.message as string}</ErrorMessage>}
                    </>
                  ))}
                </>
              ) : (
                  ...
                  {journey.usages.map((item, index) => (
                    <>
                      <Input
                        ...
                        className="mb-2 text-lg w-[500px]"
                      />
                      {errors?.usages && errors.usages[index] && <ErrorMessage>{errors.usages[index]?.message as string}</ErrorMessage>}
                    </>
                  ))}
                </>
              ) : (
                  ...
                  {journey.learningQuotes.map((item, index) => (
                    <>
                      <Input
                        ...
                        className="mb-2 text-lg w-[500px]"
                      />
                      {errors?.learningQuotes && errors.learningQuotes[index] && <ErrorMessage>{errors.learningQuotes[index]?.message as string}</ErrorMessage>}
                    </>
                  ))}
                </>
              ) : (
          ...
          <div>
            ...
            <div className="flex flex-wrap justify-center gap-5">
              {Array.from(Array(journey.length)).map((_item, index) => (
                <>
                  <Card className="p-5">
                    <Heading className="mb-2 text-xl">Action Steps for Day {index + 1}</Heading>

                    <div>
                      <Heading className="mb-1 text-lg">Major Action Step</Heading>
                      <Input
                        ...
                        defaultValue={journey.actionSteps[`day${index + 1}`].actionStep}
                      />
                      {errors.actionSteps && errors.actionSteps[`day${index + 1}`]?.actionStep && <ErrorMessage>{errors.actionSteps[`day${index + 1}`]?.actionStep?.message as string}</ErrorMessage>}
                    </div>
                    <div>
                      ...
                      />
                      {errors.actionSteps && errors.actionSteps[`day${index + 1}`]?.description && <ErrorMessage>{errors.actionSteps[`day${index + 1}`]?.description?.message as string}</ErrorMessage>}
                    </div>
                    <div>
                      <Heading className="mb-1 text-lg">Example</Heading>
                      <Textarea {...register(`actionSteps.day${index + 1}.example`)} className=" h-[100px] w-[450px]" />
                      {errors.actionSteps && errors.actionSteps[`day${index + 1}`]?.example && <ErrorMessage>{errors.actionSteps[`day${index + 1}`]?.example?.message as string}</ErrorMessage>}
                    </div>
                    <Dialog>
                      <DialogTrigger>
                        <p className="mt-3 text-xs font-medium hover:underline text-slate-600">Make additional changes</p>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle className="text-xl">Make changes in additional steps and evidences !</DialogTitle>
                        </DialogHeader>
                        <div>
                          <Heading className="mt-1 mb-1 text-lg">Additional Steps</Heading>
                          <div className="space-y-2">
                            {Array.from(Array(3).keys()).map((_item, thisIndex) => (
                              <>
                                <Input {...register(`actionSteps.day${index + 1}.additionalSteps[${thisIndex}]`)} defaultValue={journey.actionSteps[`day${index + 1}`]?.additionalSteps?.[thisIndex]} />
                              </>
                            ))}
                          </div>
                        </div>
                        <div>
                          <Heading className="mt-1 mb-1 text-lg">Evidences</Heading>
                          <div className="space-y-2">
                            {Array.from(Array(3).keys()).map((_item, thisIndex) => (
                              <>
                                <Input {...register(`actionSteps.day${index + 1}.evidences[${thisIndex}]`)} defaultValue={journey.actionSteps[`day${index + 1}`]?.evidences?.[thisIndex] || ""} />
                              </>
                            ))}
                          </div>
                        </div>
                        <p className="text-xs text-slate-400">Close for temporarily saving the changes!</p>
                      </DialogContent>
                    </Dialog>
                  </Card>
                </>
